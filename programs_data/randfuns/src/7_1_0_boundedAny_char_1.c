...
{ 
  ...
  state[6UL] = input[2UL] + (unsigned char)6;
  ...
  if (state[0UL] * state[1UL] + (state[2UL] + state[3UL]) == (state[4UL] + state[5UL]) + state[6UL] * state[7UL]) {
    state[2UL] = state[3UL] * state[7UL];
  } else {
    state[2UL] = state[1UL] + state[1UL];
  }
  output[0UL] = (state[7UL] - 8272697UL) - 656786422UL * state[2UL];
  output[1UL] = (state[0UL] + 350809882UL) + (160413798UL + state[3UL]);
  output[2UL] = (state[5UL] - state[4UL]) + (838750990UL + (state[6UL] + state[1UL]));
  output[3UL] = ((state[0UL] + state[1UL]) + state[2UL] * state[3UL]) * ((state[4UL] * state[5UL]) * (state[6UL] * state[7UL]));
}
}
